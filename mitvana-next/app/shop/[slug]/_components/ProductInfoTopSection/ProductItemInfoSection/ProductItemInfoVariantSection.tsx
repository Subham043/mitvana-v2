...
function ProductItemInfoVariantSection({ child_products }: Props) {
        ...
        {child_products.map(
          (item: ProductType["child_products"][number], index: number) => (
            <Link
              href={`/shop/${item.slug}`}
              key={index}
              className={`border-2 ${item.is_selected ? "border-[#193A43]" : "border-zinc-300"} w-25 md:w-40 cursor-pointer rounded-xl overflow-hidden`}
            >
              <div
...
export default ProductItemInfoVariantSection;
