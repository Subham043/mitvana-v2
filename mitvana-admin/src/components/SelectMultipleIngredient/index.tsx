...
import { getIngredientsHandler } from "@/utils/data/dal/ingredients";
...
type Props = {
  ...
  setSelected: (ingredient: MultiValue<OptionType> | undefined) => void;
  placeholder?: string;
};

export default function SelectMultipleIngredient({
  selected,
  setSelected,
  ...
  const loadOptions = useCallback(
    async (
      ...
      try {
        ...
        const response = await getIngredientsHandler(params, controller.signal);
        const options: OptionType[] = response.data.map((ingredient) => ({
          value: ingredient.id,
