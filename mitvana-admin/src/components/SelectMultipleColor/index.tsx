...
import { getColorsHandler } from "@/utils/data/dal/colors";
...
type Props = {
  ...
  setSelected: (color: MultiValue<OptionType> | undefined) => void;
  placeholder?: string;
};

export default function SelectMultipleColor({
  selected,
  setSelected,
  ...
  const loadOptions = useCallback(
    async (
      ...
      try {
        ...
        const response = await getColorsHandler(params, controller.signal);
        const options: OptionType[] = response.data.map((color) => ({
          value: color.id,
          label: color.name,
        }));
        return {
