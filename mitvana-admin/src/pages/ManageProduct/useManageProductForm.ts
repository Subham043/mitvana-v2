...
export function useManageProductForm({ type, id }: { type: "add" | "edit" | "clone", id?: string }) {
  ...
  useEffect(() => {
    if (type !== "add" && data && !isLoading && !isFetching && !isRefetching) {
      form.reset({
        is_update: true,
        ...
        thumbnail: undefined,
        variant: data && data.size_or_color && data.size_or_color.length > 0 ? "size" : "color",
        ...
        faqs: data && data.product_faqs && data.product_faqs.length > 0 ? data.product_faqs.map((item) => ({ question: item.question, answer: item.answer })) : [],
        is_draft: data && data.is_draft ? data.is_draft : true,
      });
    }
  }, [type, data, isLoading, isFetching, isRefetching]);
