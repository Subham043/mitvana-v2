...
export const order_razorpay_payment = mysqlTable("order_razorpay_payment", {
    ...
    status: varchar("status", { length: 255 }).notNull().default("Pending Payment"),
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at").defaultNow().onUpdateNow().notNull(),
},
    (table) => [
        check("order_razorpay_payment_status_check", sql`${table.status} IN ('Pending Payment', 'Success', 'Failed', 'Cancelled')`)
    ]);