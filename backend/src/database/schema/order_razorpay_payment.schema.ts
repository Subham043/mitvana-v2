...
export const order_razorpay_payment = mysqlTable("order_razorpay_payment", {
    ...
    razorpay_payment_id: varchar("razorpay_payment_id", { length: 255 }).notNull(),
    razorpay_payment_signature: varchar("razorpay_payment_signature", { length: 255 }).notNull(),
    status: varchar("status", { length: 255 }).notNull().default("pending"),
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at").defaultNow().onUpdateNow().notNull(),
},
    (table) => [
        check("status_check", sql`${table.status} IN ('pending', 'paid', 'failed', 'cancelled')`)
    ]);